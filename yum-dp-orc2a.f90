...
SUBROUTINE orc(it, iout)
  ...
  INTEGER :: i, it, iout
  DOUBLE PRECISION :: cl
  DOUBLE PRECISION :: clun(ix),clvn(ix),clhn(ix),clus(ix),clvs(ix),clhs(ix)
  !
  ! ------------------------------ Apply Orlanski radiation condition
  ! ------------------------------ at Northern open boundary
  DO i=1,ix
     IF (igrdu(i,jy)==0) CYCLE
     clun(i) = ( u(i,jy-1,-2)-u(i,jy-1,0) ) / ( u(i,jy-1,0)+u(i,jy-1,-2)-2.0d0*u(i,jy-2,-1) )
     IF (clun(i)<0.0d0 .OR. isnan(clun(i)) ) THEN      ! μ = 0
        u(i,jy,1) = u(i,jy,-1)
     ELSE IF (clun(i)>1.0d0) THEN ! μ = 1
        u(i,jy,1) = u(i,jy-1,0)
     ELSE                ! 0 < μ < 1
        u(i,jy,1) = ( u(i,jy,-1)*(1.0d0-clun(i))+2.0d0*clun(i)*u(i,jy-1,0) ) / ( 1.0d0+clun(i) )
     ENDIF
  ENDDO
  !
  DO i=1,ix
     IF (igrdv(i,jy)==0) CYCLE
     clvn(i) = ( v(i,jy-1,-2)-v(i,jy-1,0) ) / ( v(i,jy-1,0)+v(i,jy-1,-2)-2.0d0*v(i,jy-2,-1) )
     IF (clvn(i)<0.0d0 .OR. isnan(clvn(i)) ) THEN      ! μ = 0
        v(i,jy,1) = v(i,jy,-1)
     ELSE IF (clvn(i)>1.0d0) THEN ! μ = 1
        v(i,jy,1) = v(i,jy-1,0)
     ELSE                ! 0 < μ < 1
        v(i,jy,1) = ( v(i,jy,-1)*(1.0d0-clvn(i))+2.0d0*clvn(i)*v(i,jy-1,0) ) / ( 1.0d0+clvn(i) )
     ENDIF
  ENDDO
  !
  DO i=1,ix
     IF (igrdh(i,jy)==0) CYCLE
     clhn(i) = ( h(i,jy-1,-2)-h(i,jy-1,0) ) / ( h(i,jy-1,0)+h(i,jy-1,-2)-2.0d0*h(i,jy-2,-1) )
     IF (clhn(i)<0.0d0 .OR. isnan(clhn(i)) ) THEN      ! μ = 0
        h(i,jy,1) = h(i,jy,-1)
     ELSE IF (clhn(i)>1.0d0) THEN ! μ = 1
        h(i,jy,1) = h(i,jy-1,0)
     ELSE                ! 0 < μ < 1
        h(i,jy,1) = ( h(i,jy,-1)*(1.0d0-clhn(i))+2.0d0*clhn(i)*h(i,jy-1,0) ) / ( 1.0d0+clhn(i) )
     ENDIF
  ENDDO
  !
  ! ------------------------------ Apply Orlanski radiation condition
  ...
  DO i=1,ix
     ...
     clus(i) = ( u(i,1+1,-2)-u(i,1+1,0) ) / ( u(i,1+1,0)+u(i,1+1,-2)-2.0d0*u(i,1+2,-1) )
     IF (clus(i)<0.0d0 .OR. isnan(clus(i)) ) THEN      ! μ = 0
        u(i,1,1) = u(i,1,-1)
     ELSE IF (clus(i)>1.0d0) THEN ! μ = 1
        u(i,1,1) = u(i,1+1,0)
     ELSE                ! 0 < μ < 1
        u(i,1,1) = ( u(i,1,-1)*(1.0d0-clus(i))+2.0d0*clus(i)*u(i,1+1,0) ) / ( 1.0d0+clus(i) )
     ENDIF
  ENDDO
  !
  DO i=1,ix
     IF (igrdv(i,2)==0) CYCLE
     clvs(i) = ( v(i,2+1,-2)-v(i,2+1,0) ) / ( v(i,2+1,0)+v(i,2+1,-2)-2.0d0*v(i,2+2,-1) )
     IF (clvs(i)<0.0d0 .OR. isnan(clvs(i)) ) THEN      ! μ = 0
        v(i,2,1) = v(i,2,-1)
        v(i,1,1) = v(i,2,-1)
     ELSE IF (clvs(i)>1.0d0) THEN ! μ = 1
        v(i,2,1) = v(i,2+1,0)
        v(i,1,1) = v(i,2+1,0)
     ELSE                ! 0 < μ < 1
        v(i,2,1) = ( v(i,2,-1)*(1.0d0-clvs(i))+2.0d0*clvs(i)*v(i,2+1,0) ) / ( 1.0d0+clvs(i) )
        v(i,1,1) = ( v(i,2,-1)*(1.0d0-clvs(i))+2.0d0*clvs(i)*v(i,2+1,0) ) / ( 1.0d0+clvs(i) )
     ENDIF
  ENDDO
  !
  DO i=1,ix
     IF (igrdh(i,1)==0) CYCLE
     clhs(i) = ( h(i,1+1,-2)-h(i,1+1,0) ) / ( h(i,1+1,0)+h(i,1+1,-2)-2.0d0*h(i,1+2,-1) )
     IF (clhs(i)<0.0d0 .OR. isnan(clhs(i)) ) THEN      ! μ = 0
        h(i,1,1) = h(i,1,-1)
     ELSE IF (clhs(i)>1.0d0) THEN ! μ = 1
        h(i,1,1) = h(i,1+1,0)
     ELSE                ! 0 < μ < 1
        h(i,1,1) = ( h(i,1,-1)*(1.0d0-clhs(i))+2.0d0*clhs(i)*h(i,1+1,0) ) / ( 1.0d0+clhs(i) )
     ENDIF
  ENDDO
  !
